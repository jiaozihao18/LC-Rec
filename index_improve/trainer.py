...
class Trainer(object):
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            out, rq_loss, indices = self.model(data, use_ema=True)
    ...
    def _get_codebook_utilization(self):
        """获取码本利用率统计"""
        try:
            usage_stats = self.model.get_codebook_usage()
            # 计算平均利用率
            avg_utilization = np.mean([stat['utilization'] for stat in usage_stats])
            return avg_utilization, usage_stats
        except AttributeError:
            # 如果模型不支持 get_codebook_usage，返回 None
            return None, None
    ...
    def fit(self, data):
        ...
        for epoch_idx in range(self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # 获取码本利用率
                avg_utilization, usage_stats = self._get_codebook_utilization()
                ...
                if avg_utilization is not None:
                    valid_score_output += ", " + set_color("codebook_utilization", "blue") + ": %.4f" % avg_utilization
                    # 打印每个量化器的利用率
                    for stat in usage_stats:
                        valid_score_output += f"\n  Quantizer {stat['quantizer_id']}: {stat['utilization']:.4f} ({stat['used_codes']}/{stat['total_codes']})"
